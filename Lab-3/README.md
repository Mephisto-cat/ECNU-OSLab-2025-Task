...
### 第一层：陷阱框架
**要解决的问题：** 中断随时可能发生。发生的那一刻，CPU 正在执行的代码的寄存器状态（ra, sp, a0-a7, s0-s11...）必须完整保存，处理完中断后才能原样恢复。而且保存完状态后，必须切回**内核栈**才能安全地调 C 函数。
...
**启动第一个 tick（timer_init）：** 默认 MTIMECMP = ~0，永远不到。所以必须在 S-mode 通过内存映射方式（CLINT 地址已在 vm.c 里加了页表映射）读 MTIME 当前值、加上 INTERVAL 写入 MTIMECMP——闹钟就设好了。
...
- `uartinit`：IER 从只有 `IER_TX_ENABLE`（发送中断）扩展为 `IER_RX_ENABLE | IER_TX_ENABLE`（收发都开）
